...
$rootdir/scripts/sma.py -c <(
	cat <<- EOF
		    subsystems:
		      - name: 'vfiouser'
		        params:
		          qmp_pci_bus: 'spdk_bus'
		          qmp_address: '127.0.0.1'
		          qmp_port: 55556
		          root_path: '/var/run/vfio-user/sma'
	EOF
) &
smapid=$!
...
devid0=$(create_device 1 | jq -r '.id')
rpc_cmd nvmf_get_subsystems nqn.2016-06.io.spdk:spdk_bus:1:0

devid1=$(create_device 2 | jq -r '.id')
rpc_cmd nvmf_get_subsystems nqn.2016-06.io.spdk:spdk_bus:1:0
rpc_cmd nvmf_get_subsystems nqn.2016-06.io.spdk:spdk_bus:2:0
[[ "$devid0" != "$devid1" ]]
...
tmp0=$(create_device 1 | jq -r '.id')
tmp1=$(create_device 2 | jq -r '.id')
...
remove_device "$devid0"
NOT rpc_cmd nvmf_get_subsystems nqn.2016-06.io.spdk:spdk_bus:1:0
[[ $(rpc_cmd nvmf_get_subsystems | jq -r '. | length') -eq 2 ]]

remove_device "$devid1"
NOT rpc_cmd nvmf_get_subsystems nqn.2016-06.io.spdk:spdk_bus:2:0
...
[[ $(rpc_cmd nvmf_get_subsystems nqn.2016-06.io.spdk:spdk_bus:1:0 | jq -r '.[0].namespaces | length') -eq 1 ]]
[[ $(rpc_cmd nvmf_get_subsystems nqn.2016-06.io.spdk:spdk_bus:2:0 | jq -r '.[0].namespaces | length') -eq 0 ]]
[[ $(rpc_cmd nvmf_get_subsystems nqn.2016-06.io.spdk:spdk_bus:1:0 | jq -r '.[0].namespaces[0].uuid') == "$uuid" ]]
...
[[ $(rpc_cmd nvmf_get_subsystems nqn.2016-06.io.spdk:spdk_bus:1:0 | jq -r '.[0].namespaces | length') -eq 0 ]]
[[ $(rpc_cmd nvmf_get_subsystems nqn.2016-06.io.spdk:spdk_bus:2:0 | jq -r '.[0].namespaces | length') -eq 0 ]]
